# Ranga Nirmal Priyadarshana

\begin{center}
\small
\contacticon{\iconEmail} \href{mailto:rangap.23@cse.mrt.ac.lk}{rangap.23@cse.mrt.ac.lk}\contactsep
\contacticon{\iconPhone} +94 76 493 8927\contactsep
\contacticon{\iconWeb} \href{https://ranganp.github.io/rnp/}{Portfolio}\contactsep
\contacticon{\iconGithub} \href{https://github.com/rangaNP}{GitHub}\contactsep
\contacticon{\iconLinkedin} \href{https://www.linkedin.com/in/ranga-nirmal-984646276/}{LinkedIn}\contactsep
\contacticon{\iconLocation} Colombo, Sri Lanka
\end{center}

## Personal Statement

A highly motivated Computer Science and Engineering undergraduate at the University of Moratuwa with a strong interest in software engineering, web development, and core computer science concepts.

## Education

**BSc (Hons) in Computer Science and Engineering**
_University of Moratuwa, Sri Lanka_ | 2023 – Present

**G.C.E Advanced Level (Physical Science Stream)**
_Royal College, Colombo 07_ | 2022

- Results: 3 'A' Passes (Combined Maths, Physics, Chemistry) | Z-Score: 2.4452

**G.C.E Ordinary Level**
_Royal College, Colombo 07_ | 2019

- Results: 9 'A' Passes

**Grade 5 Scholarship Examination**
_Siddhartha Primary School_ | 2013

- Results: 190/200

## Technical Skills

- **Systems Programming:** C, C++, Rust
- **Web Development:** HTML/CSS, JavaScript, TypeScript, React, Bootstrap, Node.js, Express.js, Python, Java, Ruby
- **Application Development:** Flutter, Dart
- **Databases:** PostgreSQL, MySQL
- **DevOps & Tools:** Docker, Linux, Git, GitHub

## Open Source Contributions

**Homebrew** _(Ruby)_

- Contributor to the widely-used package manager for macOS and Linux. ([PR #21526](https://github.com/Homebrew/brew/pull/21526), [PR #21611](https://github.com/Homebrew/brew/pull/21611))

**Oh My Zsh** _(Shell)_

- Contributor to the community-driven framework for managing zsh configurations. ([PR #13556](https://github.com/ohmyzsh/ohmyzsh/pull/13556))

## Projects

**DBMS Microbanking System** _(TypeScript)_

- Developed a full‑stack university project modeling a comprehensive micro‑banking system.

**Blueberry VS Code Theme**

- Designed and published a sleek syntax highlighting theme with cool blue tones, optimized for a calm, focused developer experience.

**Pixelflip** _(JavaScript)_

- Implemented a powerful, browser-based, all-in-one image converter tool.

**4-Bit Nanoprocessor Design** _(VHDL)_

- Designed a 4-bit Nanoprocessor utilizing VHDL and successfully implemented it using Xilinx Vivado.

## Experience

**Quality Assurance Assistant**
_Swisstek Aluminium Limited_ | 2023 – 2024

- Assisted in identifying defects and improving product quality.
- Collaborated with the QA team to ensure compliance with internal standards.
- Managed technical documentation.


## Certifications

- **AWS Academy Graduate:** Microservices and CI/CD Pipeline Builder
- **AWS Academy Graduate:** Cloud Foundations

## Leadership & Activities

- **Event Committee Member:** ADScAI (2026)
- **Event Committee Member:** IEEE CIS Student Branch Chapter of UoM (2026/2027)
- **Program Committee Member:** CSE Hit the Grounds (2025)
- **Technical Committee Member:** CSE CyberCon (2026)
- **Organizing Committee Member:** RoboGames 25.26 & CSE Careers Fair (2026)

## Affiliations

- Member of OREPA (2023 – Present)